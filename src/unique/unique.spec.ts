...
describe('unique', () => {
  test('filters the unique occurrences of an array of numbers', () => {
    expect(unique([1, 4, 2, 0])).toEqual([1, 4, 2, 0])
    expect(unique([1, 1, 4, 2, 0, 2, 0])).toEqual([1, 4, 2, 0])
  })

  test('filters the unique occurrences of an array of strings', () => {
    expect(unique(['foo', 'bar', 'foo', 'foobar', 'foo', 'foo'])).toEqual(['foo', 'bar', 'foobar'])
  })

  test('filters the unique occurrences of an array of objects with an identity function', () => {
    const input = [
      { id: 1, name: 'Foo1' },
      { id: 2, name: 'Foo2' },
      { id: 1, name: 'Foo3' },
